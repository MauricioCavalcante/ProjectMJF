...
const Formulario = () => {
  ...
  const sendConfirmationEmail = async (nome, email) => {
    const brevoApiKey = process.env.REACT_APP_BREVO_KEY;
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.error("Falha ao enviar e-mail de confirmação:", response.data);
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok && data.status === "success") {
        ...
        document.getElementById("mensagem").textContent =
          ...
          await sendConfirmationEmail(formData.nome, formData.email);
